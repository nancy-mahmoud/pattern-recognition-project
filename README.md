...
An AI-powered Phishing URL Detection System that analyzes websites in real-time to determine whether they are legitimate or potential phishing threats. It utilizes a trained Machine Learning model (Random Forest Classifier) to provide accurate, confidence-based verdicts.
...
2. **Website Download**: Fetches the HTML content of the target site using a custom user-agent.
3. **Feature Extraction**: Extracts 15 specific features from the URL structure and HTML body.
4. **Preprocessing**: Normalizes the features using a pre-fitted `StandardScaler` and reduces dimensionality via `PCA`.
5. **Prediction**: The Random Forest model processes the PCA components to predict the likelihood of the site being a phishing attempt.
6. **Results Display**: Renders a comprehensive report in the UI.
...
This system uses a Machine Learning model, which is not 100% accurate. It may produce false positives or false negatives. Always verify suspicious links independently and never enter sensitive information on sites you do not fully trust.
